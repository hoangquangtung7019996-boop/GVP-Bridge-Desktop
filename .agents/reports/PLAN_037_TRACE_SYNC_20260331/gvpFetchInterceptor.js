(function () {
  ...
  console.log('[GVP Interceptor v1.21.18] ✅ Fetch interceptor installed in page context');

  const MODE_TOKEN_REGEX = /\s*--mode=\S+/gi;
  let useSpicyMode = false;
  const PROMPT_TTL_MS = 2000;
  ...
  function postBridgeMessage(type, payload) {
    try {
      // PLAN-033: Use CustomEvent to bypass isolated world postMessage barriers
      window.dispatchEvent(new CustomEvent('GVP_INTERCEPTOR_EVENT', {
        detail: { type, payload }
      }));
    } catch (error) {
      console.error('[GVP][Interceptor][Page] Failed to dispatch bridge event:', error);
    }
  }
  ...
  function handleExtensionMessage(event) {
    ...
    // We remove the strict event.source !== window check because Chrome's Isolated World 
    // to Main World postMessage often results in event.source being null or restricted.
    ...
    // Only process messages explicitly targeted at this interceptor
    if (source !== 'gvp-extension') {
      return;
    }

    log(`[Bridge] 📥 Message Received: ${type}`);
    ...
    switch (type) {
      ...
      case 'GVP_EXECUTE_DIRECT_GEN':
        log('[Bridge] 🚀 Executing delegated direct generation from page context (All Bridge)');
        // Use ORIGINAL_FETCH to bypass the interceptor and use page context auth
        ORIGINAL_FETCH('https://grok.com/rest/app-chat/conversations/new', {
          method: 'POST',
          headers: {
            'Accept': '*/*',
            'Content-Type': 'application/json'
  ...
  window.addEventListener('GVP_EXECUTE_DIRECT_GEN', (event) => {
    ...
    const { payload, statsigId } = detail;
    if (!payload) return;
    
    log('[Bridge] 🚀 Executing delegated direct generation with Header Parity');
    ...
    requestHeaders['x-xai-request-id'] = generateXaiRequestId();

    // PLAN-037: Apply Live Sentry Traces
    if (lastTraceparent) {
      requestHeaders['traceparent'] = lastTraceparent;
    }
    if (lastBaggage) {
      requestHeaders['baggage'] = lastBaggage;
    }
  ...
  window.fetch = async function (...args) {
    ...
    if (isGrokRestApi && headersToInspect) {
      ...
      // PLAN-037: Capture Live Sentry Traces
      if (hdrs['traceparent']) lastTraceparent = hdrs['traceparent'];
      if (hdrs['baggage']) lastBaggage = hdrs['baggage'];
    ...
    if (isTarget) {
      ...
      // Broadcast captured user-id for blacklisting
      if (capturedHeaders && capturedHeaders['x-userid']) {
        postBridgeMessage('GVP_FETCH_USER_ID_CAPTURED', { userId: capturedHeaders['x-userid'] });
      }
    }
