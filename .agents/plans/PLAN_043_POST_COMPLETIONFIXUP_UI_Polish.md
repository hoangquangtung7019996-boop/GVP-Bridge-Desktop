...
**Context:** The user provided UI feedback for `GalleryPanel.tsx`.
1. Thumbnails in the left/right sidebars need to be uniform squares (`aspect-ratio: 1/1`, `object-fit: cover`) so portrait/landscape images don't break the layout.
2. The Top Header needs restructuring: Top-Left becomes "GVP Desktop" with "v0.1.0" below it. The "Workspace: [ID]" text should reflect the `activeMedia()?.id`.
3. The "Open Original", "Prev/Next", and "Back to Gallery" buttons need to be centralized in the top header.

**Action 1:** REPLACE_ENTIRE_FILE
...
### 🚀 ACTION 2: INITIATE PLAN_044 (THE RUST PROXY)

While Vite hot-reloads your new square thumbnails and beautiful header, copy this prompt and feed it to the **Master Architect** to solve your video playback issues permanently.

***

**[COPY START]**
# INITIALIZE PLAN_044: THE RUST MEDIA PROXY (CORS/CSP BYPASS)

**Context:**
The UI is functioning beautifully, but media fetched from `https://assets.grok.com/...` (videos and preview images) is breaking in the Tauri frontend.
1. `assets.grok.com` does not return `Access-Control-Allow-Origin` headers, causing Tauri's strict webview to block images via CORS.
2. `assets.grok.com` attaches a strict `Content-Security-Policy: default-src 'none'` header to its media, which prevents modern browser engines (including Tauri's WebView2) from natively rendering the `<video>` tag or opening it directly.

**The Solution:**
We must proxy the media through Rust. Rust does not care about CORS or CSP headers. We need to build a Tauri custom URI scheme protocol (e.g., `gvp://proxy?url=...`).

**Your Directives for PLAN_044:**
1. In `src-tauri/Cargo.toml`, add `reqwest` (with `stream` or appropriate features if necessary) to fetch the URLs.
2. In `main.rs`, register a custom URI scheme protocol (`gvp://`) using Tauri v2's `register_uri_scheme_protocol` (or `register_asynchronous_uri_scheme_protocol`).
3. The protocol handler must:
   - Extract the target `url` from the query parameters.
   - Use `reqwest` to fetch the remote URL.
   - Strip/ignore all CSP headers from the upstream response.
   - Return the raw bytes to the Tauri frontend with `Access-Control-Allow-Origin: *` and the correct `Content-Type` (detect `.mp4` vs `.jpg`).
4. In `GalleryPanel.tsx`, create a helper function `proxify(url: string)` that wraps any `assets.grok.com` URL in `gvp://proxy?url=${encodeURIComponent(url)}` before passing it to `src` attributes.

Please write `PLAN_044.md` detailing the Find/Replace blocks to add `reqwest` to Cargo.toml, implement the custom protocol in `main.rs`, and add the `proxify` helper to the UI.
**[COPY END]**